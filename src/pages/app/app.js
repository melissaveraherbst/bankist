...
const displayMovements = function (account) {
	...
	account.movements.forEach((movement, i, arr) => {
		...
		const html = `
				...
				<div class="movements__value movements__type--${typeClass}">${formatCurrency(account.locale, account.currency, movement)}</div>
				</div>`;
	...
	if (currentAccount) {
		...
		if (currentAccount.pin === Number(inputLoginPin.value)) {
			...
			btnLoan.addEventListener("click", function (e) {
				...
				currentAccount.movementsDates.push(new Date());
				currentAccount.movementsDescriptions.push("Loan Receipt")
				updateUI(currentAccount);
				resetTimer();
			});
